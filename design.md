...
* Mark live AI tests with `@pytest.mark.integration`. Default test runs exclude them; use `uv run pytest -v -m integration` for AI research against `./input_test_data`.
